function [subsamples] = UrchinFinder(sample,plots)
...
m = a < 120; % Threshold que igual tienes que cambiar en alguna imagen
m = imdilate(m,strel('disk',9)); % Truco para unir pixeles 7
m = imfill(m,'holes'); % rellenamos huecos
m = m - bwareaopen(m, 25000); % Eliminamos puntos menores
m = bwareaopen(m,7000); % Nos quedamos con elementos grandes
m =  imclearborder(m); % Buena esa
m = imerode(m,strel('disk',2)); % Normalmente hay que deshacer algunos pasos, como el dilate 7
m = bwareaopen(m,7000); % Por seguridad
...
for i = 1:length(c)
    ...
    n{i} = imdilate(n{i},strel('disk',11));
    n{i} = imfill(n{i},'holes');
    n{i} = bwareaopen(n{i},7000);
    n{i} = imerode(n{i},strel('disk',2));
    anew{i}(~n{i}) = 255;
end
